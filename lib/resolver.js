...
/**
 * Find the next global of the given type at or after the specified absolute time.
 * @param {number} absTime - The absolute time to start searching from.
 * @param {Array} globals - An array of global objects.
 * @param {string} [type] - The type of global to search for. If omitted, any global will be considered.
 * @param {boolean} [inclusive=false] - Whether to include globals at the exact specified time.
 * @returns {Object|null} - The next global of the specified type, or null if none is found.
 */
function getNextGlobal(absTime, globals, type, inclusive = false) {
  let pos = 0;
  while (globals[pos]) {
    const isTypeMatch = !type || globals[pos].type === type;
    const isTimeMatch = inclusive
      ? globals[pos].cache.absTime >= absTime
      : globals[pos].cache.absTime > absTime;

    if (isTypeMatch && isTimeMatch) {
      return globals[pos];
    }
    pos++;
  }
...
/**
 * Find the previous global of the given type at or before the specified absolute time.
 * @param {number} absTime - The absolute time to start searching from.
 * @param {Array} globals - An array of global objects.
 * @param {string} [type] - The type of global to search for. If omitted, any global will be considered.
 * @param {boolean} [inclusive=false] - Whether to include globals at the exact specified time.
 * @returns {Object|null} - The previous global of the specified type, or null if none is found.
 */
function getPrevGlobal(absTime, globals, type, inclusive = false) {
  let pos = globals.length - 1;
  while (globals[pos]) {
    const isTypeMatch = !type || globals[pos].type === type;
    const isTimeMatch = inclusive
      ? globals[pos].cache.absTime <= absTime
...
/**
 * Retrieves all globals that occur at or immediately after a given absolute time and match a specified type.
 *
 * @param {number} absTime - The absolute time to search from.
 * @param {Array} globals - The array of global objects to search in.
 * @param {string} [type] - The type of global objects to search for. If not provided, all types will be included.
 * @param {boolean} [inclusive=false] - Whether to include globals that occur exactly at the given absolute time.
 * @returns {Array} An array containing all matching global objects that occur after the given absolute time.
 */
function getNextGlobals(absTime, globals, type, inclusive) {
  const results = [];

  let minTimeOfNextGlobal = Infinity;

  for (let i = 0; i < globals.length; i++) {
    const global = globals[i];
    const isTypeMatch = !type || global.type === type;
    const isTimeMatch = (inclusive
      ? global.cache.absTime >= absTime
      : global.cache.absTime > absTime) && global.cache.absTime <= minTimeOfNextGlobal;

    if (isTypeMatch && isTimeMatch) {
      minTimeOfNextGlobal = global.cache.absTime;
      results.push(global);
    }
  }
  return results;
...
/**
 * Retrieves all globals that occur at or immediately before a given absolute time and match a specified type.
 *
 * @param {number} absTime - The absolute time to search from.
 * @param {Array} globals - The array of global objects to search in.
 * @param {string} [type] - The type of global objects to search for. If not provided, all types will be included.
 * @param {boolean} [inclusive=false] - Whether to include globals that occur exactly at the given absolute time.
 * @returns {Array} An array containing all matching global objects that occur before the given absolute time.
 */
function getPrevGlobals(absTime, globals, type, inclusive) {
  const results = [];

  let maxTimeOfPrevGlobal = -Infinity;
  ...
  for (let i = globals.length - 1; i >= 0; i--) {
    ...
    const isTimeMatch = (inclusive
      ? global.cache.absTime <= absTime
      : global.cache.absTime < absTime) && global.cache.absTime >= maxTimeOfPrevGlobal;

    if (isTypeMatch && isTimeMatch) {
      maxTimeOfPrevGlobal = global.cache.absTime;
      results.push(global);
    }
  }
  return results;
...
function stepStateFw(state, duration, globals) {
  ...
  if (duration.isTime()) {
    ...
    var nextGlobals = getNextGlobals(state.absTime, globals);
    if (!nextGlobals.length || (nextGlobals[0].cache.absTime > state.absTime.add(s))) {
      // Simple step
      state.absTime = state.absTime.add(wn.mul(state.sPerWn));
      state.absWn = state.absWn.add(wn);
    } else {
      // Step to next global
      step = nextGlobals[0].cache.absWn.sub(state.absWn);
      state.absTime = state.absTime.add(step.mul(state.sPerWn));
      state.absWn = nextGlobals[0].cache.absWn; // avoid precision error
      for (const nextGlobal of nextGlobals) {
        maybeUpdateState(state, nextGlobal);
      }
      // Call recursively with resting duration
      rest = wn.sub(step);
      if (rest > 0) {
...
function stepStateBw(state, duration, globals) {
  ...
  if (duration.isTime()) {
    ...
    var prevGlobals = getPrevGlobals(state.absTime, globals);
    if (!prevGlobals.length || (prevGlobals[0].cache.absTime < state.absTime.add(s))) {
      // Simple step
      ...
      // Step to next global
      step = prevGlobals[0
      ].cache.absWn.sub(state.absWn);
      state.absTime = state.absTime.add(step.mul(state.sPerWn));
      state.absWn = prevGlobals[0].cache.absWn; // avoid precision error
      for (const prevGlobal of prevGlobals) {
        maybeUpdateState(state, prevGlobal);
      }
      // Call recursively with resting duration
      rest = a.sub(step);
      if (rest < 0) {
