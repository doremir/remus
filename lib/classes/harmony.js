...
export default class Harmony extends Item {
  ...
  parallel() {
    ...
    if (this.isMajor()) {
      return new Harmony(this.getRootPitch().interval('m3', 'down'), 'minor');
    } else {
      return new Harmony(this.getRootPitch().interval('m3', 'up'), 'major');
    }
  }
  ...
  toString() {
    ...
    let root = rootPitch.text || (rootPitch.name().toUpperCase() + rootPitch.accidental());
    ...
    if (bassPitch) {
      if (bassPitch.text) {
        ...
        bass = '/' + bassPitch.name().toUpperCase() + bassPitch.accidental();
      }
    }
...
Harmony.fromString = function(str, parent) {
  ...
  if (matches) {
    ...
    // Sevenths
      ...
      kind = 'major-minor';
      override[9] = 'M';
    } else if (rest.match(/^(\-|m|mi|min)[\/\(]?(\\+7|maj(?!9)|M7?|Δ|Δ7|maj7)/)) {
      kind = 'major-minor';
