...
// Cache keys that contain cyclic Item references after resolve().
// Set by Event.resolveEvents(): children (Item[]), prevSibling/nextSibling (Item),
// globals (shared meta array that references back into the tree).
const CACHE_SKIP_KEYS = new Set(['children', 'prevSibling', 'nextSibling', 'globals']);
...
export default class Item {
  constructor(properties = {}, parent, copy) {
    ...
    if (properties.type && this.constructor.itemType
        && properties.type !== this.constructor.itemType) {
      console.warn('%s constructor was sent object with specified type %s', this.constructor.itemType, properties.type);
    }
  ...
  toJSON() {
    ...
    if (this.cache && Item.serializeCache) {
      const rawCache = Item.serializeCache === true
        ? this.cache
        : _.pick(this.cache, Item.serializeCache);
      // Serialize cache values, skipping internal tree-structure refs
      const serializedCache = {};
      for (const [k, val] of Object.entries(rawCache)) {
        if (CACHE_SKIP_KEYS.has(k)) continue;
        if (val === this) continue; // break self-reference (e.g. Key.cache.key === Key)
        if (val === undefined || val === null) continue;
        if (_.isObject(val) && val.toJSON) {
          serializedCache[k] = val.toJSON();
        } else {
          serializedCache[k] = val;
        }
      }
      obj.cache = serializedCache;
    }
