...
export default class Chord extends Event {
  ...
  localResolve() {
    ...
    if (this.harmony) {
      this.cache.harmony = this.harmony;
      this.cache.harmonyKind = this.harmony.kind();
    } else if (this.text) {
      // No explicit harmony — try to parse the text into one
      try {
        const parsed = Harmony.fromString(this.text);
        if (parsed && parsed.root) {
          this.cache.harmony = parsed;
          this.cache.harmonyKind = parsed.kind();
        }
      } catch (e) {
        // Text is not a valid chord (e.g. "Test") — leave cache.harmony null
      }
    }
  }
}
