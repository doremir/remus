...
function generateChordPlayback(chordSequence, patternSong) {
  ...
  // Sort events by time position so the forward-only chord advancement works correctly
  var sortedEvents = container.events.slice().sort((a, b) => a.cache.absWn.sub(b.cache.absWn));
