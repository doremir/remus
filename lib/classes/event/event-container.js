...
export default class EventContainer extends Event {
  ...
  static getSlots() {
    return Object.assign({
      eventTimeMode: {
        type: String,
        default: this.defaultEventTimeMode
      },
      metaTimeMode: {
        type: String,
        default: this.defaultMetaTimeMode
      },
      events: {
        type: ItemHandler.ArrayOf.Event,
        default: [],
        owned: true
      },
      metas: {
        type: ItemHandler.ArrayOf.Meta,
        default: [],
        owned: true
      },
      scale: {
  ...
  constructor(properties, parent) {
    ...
    this.initEvents(properties.events || [], this.constructor.defaultEventItemType); // Why not just specify events and metas as slots??
  ...
  toJSON() {
    ...
    if (obj.scale instanceof Fraction) obj.scale = obj.scale.toFraction();
    if (obj.eventTimeMode === this.constructor.defaultEventTimeMode) delete obj.eventTimeMode;
    if (obj.metaTimeMode === this.constructor.defaultMetaTimeMode) delete obj.metaTimeMode;
    obj.events = _.map(this.childEvents(), event => event.toJSON());
    let metas = this.childMetas();
    if (metas && (metas.length > 0))
      obj.metas = _.map(metas, meta => meta.toJSON());
    else
      delete obj.metas;
    return obj;
  }
...
EventContainer.defaultEventTimeMode = 'relative';
EventContainer.defaultMetaTimeMode = 'relative';
