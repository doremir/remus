...
export default class Score {
  constructor(params) {
    ...
    this.width = params.width || (songScoreProperties.pageWidth * 3.83) || 700;
    this.height = params.height || (songScoreProperties.pageHeight * 3.83) || 1200;
    this.margins = {};
  ...
  updateScore() {
    ...
    for (let part of parts) {
      ...
      for (let i = 0; i < part.staves; i++) {
        staves.push({id: staff, staffLines: part.staffLines}); // voice.kind === 'single-drum' ? 1 : 5});
        staff++;
      }
    }
  ...
  updatePaging() {
    ...
    let maxY = this.height - (this.margins.bottom || 0);
    let startY = minY;
    ...
    if (this.pageItems) {
      for (let pageItem of this.pageItems) {
        ...
        if (pageItem.avoidCollisions === "songTitle" && pageItem.page === 1) {
          startY = Math.max(startY, pageItem.maxY + 20);
        }
      }
    }
...
function getPageOrigin (score, originX, originY) {
  ...
  switch (originX) {
    ...
    case "right":        x = score.width; break;
    case "rightMargin":  x = score.width - (score.margins.right || 0); break;
    case "pageCenter":   x = score.width / 2; break;
    case "marginCenter": x = leftMargin + ((score.width - leftMargin - (score.margins.right || 0)) / 2); break;
  }
  let topMargin = score.margins.top || 0;
  switch (originY) {
    case "top":          y = 0; break;
    case "topMargin":    y = topMargin; break;
    case "bottom":       y = score.height; break;
    case "bottomMargin": y = score.height - (score.margins.bottom || 0); break;
    case "pageCenter":   y = score.height / 2; break;
    case "marginCenter": y = topMargin + ((score.width - topMargin - (score.margins.bottom || 0)) / 2); break;
  }
  return [x, y];
}
...
function convertPageItemPosition (score, pageItem) {
  ...
  return [baseX + pageItem.pageX, baseY + pageItem.pageY];
}

function createScorePageItem (score, pageItem, page) {
  let [x, y] = convertPageItemPosition(score, pageItem);
  ...
  return new ScorePageText(pageItem, score, {
    ...
    pageX: pageItem.pageX,
    pageY: pageItem.pageY,
    page: pageItem.page,
