...
export default class ScoreTie extends ScoreMeta {
  updatePosition() {
    ...
    let staffY = note.scoreNoteChord.globalStaff * note.score.staffDistance;
    this.y = staffY + staffSpace * ((this.yOffset || 0) + note.visibleYPos());

    // Have tie directions flip when crossing the average staff line of a chord.
    // TODO: Should perhaps be implemented in a different place.
    if (note.scoreNoteChord.notes.length > 1) {
      let averageStaffLine = note.scoreNoteChord.notes.map(n => n.staffLine).reduce((a, b) => a + b, 0) / note.scoreNoteChord.notes.length;
      this.direction = (note.staffLine > averageStaffLine) ? 'down' : 'up';
    }
  }
