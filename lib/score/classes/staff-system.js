export default class StaffSystem {
  constructor(score, num, params = {}) {
    ...
    this.visibleParts = params.visibleParts;
  }
  ...
  toSVG(svg) {
    ...
    let y = this.y;
    let delta = this.staffSize * 0.25;
    let staffYTable = this.staffYTable;


    // Create SVG group for the staff system, with a sane ID
    let staffSystem = svg.group().id('staff-system-' + this.num).x(0).y(y);

    // Draw staff lines
    for (let staff = 0; staff < this.staves.length; staff++) {
      let staffY = staffYTable[staff];
    ...
    for (let item of this.items) {
      ...
      group.addClass(item.constructor.itemType);
